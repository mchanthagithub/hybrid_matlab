function [ vel_new, momentum_new ] = updatePointVelocitiesHybrid( physics_grid_min, physics_grid_delta,...
...
% Rasterize momentum to grid
    for pt_num = 1:mpm_points_num_points
        if(~dem_coupled(pt_num))
%             vel_new(pt_num,:) = mpm_points_vel(pt_num,:);
